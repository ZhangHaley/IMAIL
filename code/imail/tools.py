...
def video_summary(name, video, step=None, fps=20):
  print(name)
  name = name if isinstance(name, str) else name.tostring().decode('utf-8')
...
def simulate(agent, envs, steps=0, episodes=0, state=None,render=False):
...
def save_episodes(directory, episodes):
  mp4_file = 'video/idealgas0.mp4'
  directory = pathlib.Path(directory).expanduser()
  directory.mkdir(parents=True, exist_ok=True)
  timestamp = datetime.datetime.now().strftime('%Y%m%dT%H%M%S')
  for episode in episodes:
    add_background(episode,mp4_file)
...
def save_buffer_episodes(directory, episodes):
  directory = pathlib.Path(directory).expanduser()
  directory.mkdir(parents=True, exist_ok=True)
  for episode in episodes:
    length = len(episode['reward'])
    filename = directory / f'buffer-{length}.npz'
    with io.BytesIO() as f1:
      np.savez_compressed(f1, **episode)
      f1.seek(0)
      with filename.open('wb') as f2:
...
def convert_episodes(directory,target_dir,mp4_file):
  directory = pathlib.Path(directory).expanduser()
  ...
  for filename in directory.glob('*.npz'):
    if filename not in cache:
      ...
      identifier = str(uuid.uuid4().hex)
      length = len(episode['reward'])
      filename = target_dir / f'{timestamp}-{identifier}-{length}.npz'
      with io.BytesIO() as f1:
        np.savez_compressed(f1, **episode)
        f1.seek(0)
        with filename.open('wb') as f2:
...
def convert_episodes_images(directory,target_dir,images_file):
  directory = pathlib.Path(directory).expanduser()
  target_dir = pathlib.Path(target_dir).expanduser()
  timestamp = datetime.datetime.now().strftime('%Y%m%dT%H%M%S')
  cache = {}
  for filename in directory.glob('*.npz'):
    if filename not in cache:
      try:
        ...
        continue
      cache[filename] = episode
      add_background_images(episode,images_file)
      identifier = str(uuid.uuid4().hex)
      length = len(episode['reward'])
      filename = target_dir / f'{timestamp}-{identifier}-{length}.npz'
      with io.BytesIO() as f1:
...
def add_background(episode,mp4_file,render = False):
  files = glob.glob(os.path.expanduser(mp4_file))
  meta_img = episode['image'].copy()
  shape2d = meta_img[0].shape[:2]
  bg_source = natural_imgsource.RandomVideoSource(shape2d, files, grayscale=False, total_frames=1000)
  for i in range(len(meta_img)):
    dm_image = meta_img[i]
    mask = np.logical_and((dm_image[:, :, 2] > dm_image[:, :, 1]), (dm_image[:, :, 2] > dm_image[:, :, 0]))  # hardcoded for dmc
    bg = bg_source.get_image()
    dm_image[mask] = bg[mask]
    meta_img[i] = dm_image
    if render:
      show_render(episode['image'][i])
      show_render(meta_img[i])
  episode['image_paired'] = meta_img

def add_background_images(episode,images_file,render = False):
  files = glob.glob(os.path.expanduser(images_file))
  meta_img = episode['image'].copy()
  shape2d = meta_img[0].shape[:2]
  bg_source = natural_imgsource.RandomImageSource(shape2d, files, grayscale=False,total_frames=1)
  for i in range(len(meta_img)):
    dm_image = meta_img[i]
    mask = np.logical_and((dm_image[:, :, 2] > dm_image[:, :, 1]), (dm_image[:, :, 2] > dm_image[:, :, 0]))  # hardcoded for dmc
    bg = bg_source.get_image()
